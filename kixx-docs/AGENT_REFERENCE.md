...
| URL Path | File Location | Explanation |
|----------|---------------|-------------|
| `/` | `pages/page.jsonc` | Root path, "index" stripped → empty → `page.jsonc` |
| `/about` | `pages/about.jsonc` OR `pages/about/page.jsonc` | Direct mapping or nested |
| `/about/index` | `pages/about.jsonc` OR `pages/about/page.jsonc` | "index" stripped → same as `/about` |
| `/blog/post` | `pages/blog/post.jsonc` OR `pages/blog/post/page.jsonc` | Nested paths work |

**Algorithm**:
1. Take URL pathname
2. Remove file extensions
3. Remove "index" from path segments
4. Append `page.jsonc` if looking for metadata

## File Structure

```
pages/
├── page.jsonc          # Homepage metadata (NOT index.jsonc)
├── page.md             # Homepage content
├── about.jsonc         # /about metadata
├── about.md            # /about content
└── blog/
    ├── page.jsonc      # /blog homepage
    ├── page.md         # /blog content
    ├── post1.jsonc     # /blog/post1 metadata
    └── post1.md        # /blog/post1 content
```
